...
import com.dmd.mall.config.AlipayConfig;
import com.dmd.mall.service.ALiservice;
...
public class ALiserviceImpl implements ALiservice {
    ...
    public Map pay(String total_fee, String oid) throws Exception {
        ...
        try {
            ...
            //随机生成商户订单号
            String out_trade_no = "ali" + System.currentTimeMillis();
    ...
    @Override
    public Map recharge(String total_fee) throws Exception {
        ...
        try {
            ...
            model.setProductCode("QUICK_MSECURITY_PAY");//销售产品码
            payRequest.setBizModel(model);          //封装好的额外参数放到主请求中
            payRequest.setNotifyUrl(alipayConfig.getDeposit_url());//设置异步通知地址
            //支付宝请求应答
            AlipayTradeAppPayResponse responses = client.sdkExecute(payRequest);
            System.out.println("-----------------------------支付宝应答处理开始------------------------------------");
            System.out.println(responses.getBody()); // 就是orderString 可以直接给客户端请求，无需再做处理
    ...
    public void alipayNotifyApp(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (trade_status.equals("TRADE_FINISHED") || trade_status.equals("TRADE_SUCCESS")) {
            ...
            //2.判断登录用户是教练还是用户
            //3.流水账表新增信息：充值价格，充值时间，"支付宝充值"，"充值类型1(收入)"，用户类型
            System.out.println("---------------------------支付成功--------------------------");
        }
        responseBody(response, resultFail);
        return;
